...
import 'package:smartkit_pro/ui/fullApps/foodMaster/helper/drawerstyle/drawer_scaffold.dart' as drawer;
...
class _SideDrawerState extends State<SideDrawer> with TickerProviderStateMixin {
  ...
  Widget createMenuItems(drawer.MenuController menuController) {
    ...
    if (widget.child != null) {
      ...
      final perListItemDelay =
          menuController.state != drawer.MenuState.closing ? 0.15 : 0.0;
      final millis = menuController.state != drawer.MenuState.closing
  ...
  buildListItem(
      drawer.MenuController menuController,
    Menuitem item,
  ...
  Widget createDrawer(drawer.MenuController menuController) {
    List<Widget> widgets = [];
    ...
    drawer.MenuController controller = drawer.DrawerScaffold.currentController(context);
    return Transform(
    ...
    textStyle = widget.textStyle ??
        Theme.of(context).textTheme.subtitle1!.copyWith(
            color: widget.color.computeLuminance() < 0.5
                ? Colors.white
                : Colors.black);
    return drawer.DrawerScaffoldMenuController(
        direction: widget.direction,
        builder: (BuildContext context, drawer.MenuController menuController) {
          ...
          if (menuController.state == drawer.MenuState.closed ||
              menuController.state == drawer.MenuState.closing ||
              selectorYTop == null) {
...
class AnimatedMenuListItem extends ImplicitlyAnimatedWidget {
  ...
  final drawer.MenuState? menuState;
...
class _AnimatedMenuListItemState
  ...
  void forEachTween(TweenVisitor<dynamic> visitor) {
    ...
    switch (widget.menuState!) {
      case drawer.MenuState.closed:
      case drawer.MenuState.closing:
        slide = closedSlidePosition;
        opacity = 0.0;
        break;
      case drawer.MenuState.open:
      case drawer.MenuState.opening:
